...
function sys_ShowListPage($num,$pagenum,$dolink,$dotype,$page,$lencord,$ok,$search=""){
	...
	$pagelink=$pagetop.$pagepri.$pagenext.$pageeof;
	 //数字分页
	 if($pagenum == 1){
          $numpageeof = "<a href='".$dolink."index".$dotype."'  class='current'>1</a>";
     }else{
     	$numpageeof = "<a href='".$dolink."index".$dotype."'>1</a>";
     } 
	if($page<=9){
		
		for($i=2;$i<=$page;$i++){
			if($pagenum==$i){
		         $numpageeof .= "<a href='".$dolink."index_".$i.$dotype."'   class='current'>".$i."</a>";
		    }else{
		    	$numpageeof .= "<a href='".$dolink."index_".$i.$dotype."'>".$i."</a>";
		    }
		}
	}
	...
	if($page>9){ 
		if($pagenum<6){
			...
			$jiannumpage = $page - 1;
			if($jiannumpage==$pagenum){
			     $numpageeof.= "...<a href='".$dolink."index_".$jiannumpage.$dotype."'   class='current'>".$jiannumpage."</a>";
			}else{
				$numpageeof.= "...<a href='".$dolink."index_".$jiannumpage.$dotype."'>".$jiannumpage."</a>";
			}
			if($pagenum==$page){
			   $numpageeof.= "<a href='".$dolink."index_".$page.$dotype."' class='current'>".$page."</a>";
			}else{
				$numpageeof.= "<a href='".$dolink."index_".$page.$dotype."'>".$page."</a>";
			}
		}else{
			$lnum = $page - $pagenum;
            for($i=2;$i<=3;$i++){
			...
			if($lnum<=2){
				$pagenuml = intval($pagenum)-1;
				$numpageeof .= "...";
				for($i=$pagenuml;$i<=$page;$i++){
					if($pagenum==$i){
				         $numpageeof .= "<a href='".$dolink."index_".$i.$dotype."'   class='current'>".$i."</a>";
				    }else{
				    	$numpageeof .= "<a href='".$dolink."index_".$i.$dotype."'>".$i."</a>";
				...
				$pagenuml = intval($pagenum)-1;
				$pagenumr = intval($pagenum)+1;
				$numpageeof .= "...";
				for($i=$pagenuml;$i<=$pagenumr;$i++){
					if($pagenum==$i){
				         $numpageeof .= "<a href='".$dolink."index_".$i.$dotype."'   class='current'>".$i."</a>";
				    }else{
				    	$numpageeof .= "<a href='".$dolink."index_".$i.$dotype."'>".$i."</a>";
	...
	if($pagenum<$page){
		$pagenumr = intval($pagenum)+1;
		$numpageeof.= "<a href='".$dolink."index_".$pagenumr.$dotype."' title='下一页'>></a>";
	}  

	//替换模板变量
	$pager['showpage']=ReturnListpageStr($pagenum,$page,$lencord,$num,$numpageeof,$options);

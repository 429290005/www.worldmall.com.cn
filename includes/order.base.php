...
class BaseOrder extends Object
{
    ...
    function _get_my_address($user_id)
    {
        ...
        return $address_model->get_list($user_id);
    }
    ...
    function _get_shipping_methods($store_id)
    {
        ...
        $data = $shipping_model->find('enabled=1 AND store_id=' . $store_id);
        foreach ($data as $key => $row) {
            $data[$key]['cod_regions'] = unserialize($row['cod_regions']);
            $data[$key]['cod_first_price'] = unserialize($row['cod_first_price']);
            $data[$key]['cod_step_price'] = unserialize($row['cod_step_price']);
        }
        return $data;
    }
    ...
    {
        /* 验证收货人信息填写是否完整 */
        $addr_id = intval($post['address_options']);
        if ($addr_id > 0) {
            $address_mod =& m('address');
            $address = $address_mod->get_info($addr_id);
            $post = array_merge($post, $address);
        } else {
            $region_mod =& m('region');
            $region = $region_mod->get($post['region_id']);
            $post['parent_id'] = $region['parent_id'];
        }
        ...
        $weigth = floatval($goods_info['weight']);
        $shipping_fee = 0;
        
        if ($weigth > 0) {
            /* 按默认配送 */
            $shipping_fee = ($weigth <= 1) ? $shipping_info['first_price'] : ($shipping_info['first_price'] + ceil($weigth - 1) * $shipping_info['step_price']);
            
            // 按地区配送
            $cod_regions = unserialize($shipping_info['cod_regions']);
            $cod_first_price = unserialize($shipping_info['cod_first_price']);
            $cod_step_price = unserialize($shipping_info['cod_step_price']);
            if ($cod_regions[$post['parent_id']]) {
                $shipping_fee = ($weigth <= 1) ? $cod_first_price[$post['parent_id']] : ($cod_first_price[$post['parent_id']] + ceil($weigth - 1) * $cod_step_price[$post['parent_id']]);
            }
        }

...
class SessionProcessor
{
    ...
    function SessionProcessor(&$db, $session_table, $session_data_table, $session_name = 'ECM_ID', $session_id = '')
    {
        ...
        $this->max_life_time = $this->max_life_time ? $this->max_life_time : 1440;
    ...
    function _sess_gc()
    {
        ...
        if (!empty($this->_related_tables)) {
            foreach ($this->_related_tables as $_t) {
                $_t['alias'] = $_t['alias'] ? $_t['alias'] : 't';
                $_t['ext_limit'] = $_t['ext_limit'] ? ' AND ' . $_t['ext_limit'] : '1 = 1';
                $sql = "DELETE {$_t['alias']} FROM {$_t['name']} AS {$_t['alias']} 
                		INNER JOIN {$this->session_table} AS s ON {$_t['alias']}.{$_t['ref_key']} = s.sesskey 
                		WHERE s.expiry < {$time}{$_t['ext_limit']}";
                $this->db->query($sql);
            }
        }

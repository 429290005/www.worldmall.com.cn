...
/**
 * 获取提现状态相应的文字表述
 * @param string $cash_status
 */
function cash_status($cash_status)
{
    $lang_key = '';
    switch ($cash_status)
    {
        case 'waiting': // 等待处理
            $lang_key = 'cash_waiting';
        break;
        case 'succeed': // 处理完成
            $lang_key = 'cash_succeed';
        break;
        case 'fail':    // 申请失败
            $lang_key = 'cash_fail';
        break;
        case 'cancel':  // 取消申请
            $lang_key = 'cash_cancel';
        break;
    }
 ...
 *    @return    array
 */
function list_widget($template_name = 'default')
{
    $widget_dir = ROOT_PATH . '/external/widgets/' . $template_name;
    static $widgets    = null;
    if ($widgets === null)
    {
        $widgets = array();
        ...
        while (false !== ($entry = $dir->read()))
        {
            ...
            $info = get_widget_info($entry, $template_name);
            $widgets[$entry] = $info;
        }
    }
 ...
 *    @return    array
 */
function get_widget_info($name, $template_name)
{
    $widget_info_path = ROOT_PATH . '/external/widgets/' . $template_name . '/' . $name . '/widget.info.php';
...
function refund_status($refund_status)
{
    ...
    {
        case 1:
            $lang_key = 'refund_waiting';
            break;
        case 2:
            $lang_key = 'refund_seller_agree_rg';
            break;
        case 3:
            $lang_key = 'refund_buyer_has_rg';
            break;
        case 4:
            $lang_key = 'refund_seller_has_agree';
            break;
        case 5:
            $lang_key = 'refund_seller_not_disagree';
            break;
        case 6:
            $lang_key = 'refund_seller_has_disagree';
            break;
        case 7:
            $lang_key = 'refund_seller_agree';
            break;
        case 8:
            $lang_key = 'refund_succeed';
            break;
        case 9:
            $lang_key = 'refund_fail';
            break;
        case 10:
            $lang_key = 'refund_cancel';
            break;
        case 11:
            $lang_key = 'refund_seller_disagree';
            break;
    }
 ...
 * @param string $key
 */
function goods_category($key)
{
    if (!$key) return '';
    
    return Lang::get('category_'.$key);
}
?>
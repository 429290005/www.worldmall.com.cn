...
class BaseModel extends Object
{
    ...
    function find($params = array())
    {
        ...
        /* 组合(GROUP BY)*/
        $field_group = $this->getRealFields($group);
        $group && $group = ' GROUP BY ' . $field_group;
        ...
        if ($count) {
            $count_sql = "SELECT COUNT(*) as c FROM {$tables}{$conditions}";
            if ($group) {
                $count_sql = "SELECT COUNT(*) as c FROM (SELECT {$field_group} FROM {$tables}{$conditions}{$group}) AS SUM";
            }
            $this->_updateLastQueryCount($count_sql);
        }
        
        /* 完整的SQL */
        $sql = "SELECT {$fields} FROM {$tables}{$conditions}{$group}{$order}{$limit}";

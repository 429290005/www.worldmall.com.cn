...
//            $data['page_size']              = $_POST['page_size'];
            $data['close_register']         = $_POST['close_register'];
    ...
    function email_setting()
    {
        ...
        else
        {
            ...
            $data['email_open']     = intval($_POST['email_open']);
            $model_setting->setAll($data);
...
//            $data['store_allowed_files']    = $_POST['store_allowed_files'];
            $data['order_receiving_endtime'] = intval($_POST['order_receiving_endtime']);
            $data['lowest_cash_money'] = round($_POST['lowest_cash_money'], 2);
            $data['goods_update_time_alert'] = intval($_POST['goods_update_time_alert']);
            $data['ticheng_ratio'] = round($_POST['ticheng_ratio'], 2);
            $model_setting->setAll($data);
    ...
    function self_setting()
    {
        ...
        {
            $this->assign('setting', $setting);
            $this->display('setting.self_setting.html');
        }
        else
        {
            $data['self_store_qq']       = trim($_POST['self_store_qq']);
            $data['self_store_wangwang'] = trim($_POST['self_store_wangwang']);
            $data['self_store_region']   = trim($_POST['self_store_region']);
            $data['self_store_address']  = trim($_POST['self_store_address']);
            $data['self_store_tel']      = trim($_POST['self_store_tel']);
            $model_setting->setAll($data);

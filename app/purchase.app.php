...
//        echo $sort;
        ...
        if ($_GET['export_all']) {
			...
			foreach ($data as $key=>$row) {
				...
				$data[$key]['count_purchase_sn']=$purchase_sn==$data[$key]['purchase_sn']?(int)$data[$k]['count_purchase_sn']+$data[$key]['quantity']:$row["quantity"];
//				$data[$key]['count_moneys']=$count_money==$data[$key]['money']?$row["money"]+$data[$key-1]['money']:$row["money"];
				$data[$key]['count_moneys']=$purchase_sn==$data[$key]['purchase_sn']?(float)$data[$k]['count_moneys']+$data[$key]['money']:$row["money"];
				if($purchase_sn==$data[$key]['purchase_sn']){
					//unset($data[$k]['count_purchase_sn']);
					//unset($data[$k]['count_moneys']);
				}
//				var_dump($count_money);
//				var_dump($data[$key]['money']);
				$k = $key;
...
//			exit();
			...
			array_unshift($data,array('商品入库汇总明细表'),array($date_show),array('序号','入库日期', '采购单号', '供应商','供应商货号','品名','类别','颜色','规格','单位','入库数量','成本单价','入库金额','备注','数量小计','金额小计'));
        ...
        if (!IS_POST)
        {
            ...
            if (file_exists($goods_list_file)) {
				...
				if($goods_num>5000){
					...
					$goods_list_two = array_chunk($goods_list,1000,true);
					$cache_data .= "\r\nreturn array (\n";
					
					for($i=0;$i<count($goods_list_two);$i++){
						$new_cache_data =  var_export($goods_list_two[$i],true);
						$new_cache_data = substr($new_cache_data,8);
						$cache_data .= substr($new_cache_data,0,strlen($new_cache_data)-1);
						file_put_contents($goods_list_file, $cache_data,FILE_APPEND);
						unset($cache_data);

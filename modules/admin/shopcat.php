<?php  
 class shopcat extends spController  {  
	...
	function index(){
		...
		$this->display('admin/shopcat_list.html');
	}
	//添加页面
	function addpage(){
		
		$this->cat_select = $this->getCate(0);
		$this->display('admin/shopcat_add.html');
	}
	//添加动作
	function insert(){
		...
		if(!empty($upfile)){
			$pic=upload('shopcat','jpg,gif,rar,pdf',2048000000,1,'348*177',0);
		}
		//将获取的数据转成数组，添加到数据库中
		$newrow = array(
			'short'      => $short,
		...
		//运行添加的动作
		if(spClass('shop_cat')->create($newrow)){
			$link[0]['text'] = '继续添加';
			$link[0]['href'] = spUrl("shopcat","addpage",array("pid"=>$pid));
			$link[1]['text'] = '返回文章分类列表';
			$link[1]['href'] = spUrl("shopcat","index",array("pid"=>$pid));
	...
	function editpage(){
		...
		$this->pid=$pid;
		$this->cat = spClass('shop_cat')->find($condition,'id asc');
		// print_r($cat);
		// exit();
		$this->cat_select = $this->getCate(0);
		$this->display('admin/shopcat_edit.html');	
	}
	//修改数据
	function update(){
		//获取不到id就返回错误页面
		...
		if(!empty($upfile)){
			...
			//删除更新前的图片
			$row=spClass('shop_cat')->find('id='.$id);
			unlink("uploads/shopcat/".$row['upfile']);
			unlink("uploads/sm_shopcat/".$row['upfile']);
			unlink("uploads/smm_shopcat/".$row['upfile']);
		}
		//运行更新的动作
		if(spClass('shop_cat')->update($condition,$updaterow)){
			$link[0]['text'] = '返回编辑页面';
			$link[0]['href'] = spUrl("shopcat","editpage",array("id"=>$id,"pid"=>$pid));
			$link[1]['text'] = '返回文章分类列表';
			$link[1]['href'] = spUrl("shopcat","index",array("id"=>$id,"pid"=>$pid));
			$this->sys_msg("文章分类修改成功！",0,$link);	
		}else{
			$this->error("操作失败，请联系网站管理员",spUrl("shopcat","index"));	
		}
	}
	//删除操作
	function delete(){
		...
		$condition = array('id' => $id);
		if($row=spClass('shop_cat')->delete($condition)){
			$this->error("文章分类删除成功！",spUrl("shopcat","index",array("pid"=>$pid)));

 ...
 class member extends spController  {  
	...
	function index(){
		...
		if(!empty($keyword) and !empty($flag)){
			$sql="SELECT * FROM sp_member WHERE username like '%".$keyword."%' and flag=".$flag."";	
		}else if(!empty($keyword)){
			$sql="SELECT * FROM sp_member WHERE username like '%".$keyword."%'";	
		}else if(!empty($flag)){
		...
		$this->display('admin/member_list.html');
	}
	//会员菜谱列表
	...
	function addpage(){
		...
		$this->display('admin/member_add.html');
	}
	//添加动作
	function insert(){
		...
		if($row){
			$link[0]['text'] = '继续添加';
			$link[0]['href'] = spUrl("member","addpage");
			$link[1]['text'] = '返回文章列表';
			$link[1]['href'] = spUrl("member","index");
	...
	function editpage(){
		...
		$this->display('admin/member_edit.html');	
	}
	//修改数据
	function update(){
		//获取不到id就返回错误页面
		$id  = empty($_REQUEST['id'])?$this->error('程序出现错误！'):$_REQUEST['id'];
		//要更新的字段
		$flag    	= empty($_POST['flag'])?0:$_POST['flag'];
		$is_open    = empty($_POST['is_open'])?0:$_POST['is_open'];
		$endtime=0;
		switch($flag){
			case 1:
				$endtime=time()+(60*60*24*30*1);
				break;
			case 2:
				$endtime=time()+(60*60*24*30*2);
				break;
			case 3:
				$endtime=time()+(60*60*24*30*3);
				break;
			case 4:
				$endtime=time()+(60*60*24*30*6);
				break;
			default:
				$endtime=0;
		}
		...
		$now=time();
		spClass('member_list')->update('endtime<'.$now,$updaterow2);
		...
		if($row){
			$link[0]['text'] = '返回修改页面';
			$link[0]['href'] = spUrl("member","editpage",array("id"=>$id));
			$link[1]['text'] = '返回会员列表';
			$link[1]['href'] = spUrl("member","index");
	...
	function delete(){
		...
		if($row=spClass('member_list')->delete($condition)){
			$this->error("删除成功!",spUrl("member","index"));	
		}else{
			$this->error("操作失败，请联系网站管理员",spUrl("member","index"));		
		}
	}
	//批量处理文章
	function batch(){
		if(empty($_POST['type'])){$this->error("请选择动作！",spUrl("member","index"));}
		if(empty($_POST['checkboxs'])){$this->error("请选择文章！",spUrl("member","index"));}
		...
		switch($type){
			case 'button_remove';
				...
				if(spClass("member_list")->query($sql)){
					$this->error("批量删除成功!",spUrl("member","index"));	
				}
			break;
			case 'button_hide';	
				$sql="UPDATE sp_member SET is_open=0 WHERE id IN(".$id.")";
				if(spClass("member_list")->query($sql)){
					$this->error("批量隐藏成功!",spUrl("member","index"));	
				}
			break;
			case 'button_show';	
				$sql="UPDATE sp_member SET is_open=1 WHERE id IN(".$id.")";
				if(spClass("member_list")->query($sql)){
					$this->error("批量显示成功!",spUrl("member","index"));	
				}
			break;
		}
	}
	...
	function memberbatch(){
		if(empty($_POST['type'])){$this->error("请选择动作！",spUrl("member","memberproduct"));}
		if(empty($_POST['checkboxs'])){$this->error("请选择菜谱！",spUrl("member","memberproduct"));}
		$id=implode(',', $_REQUEST['checkboxs']);
		$type = $_POST['type'];
		switch($type){
			case 'button_remove';
				...
				if(spClass("member_product")->query($sql)){
					$this->error("批量删除成功!",spUrl("member","memberproduct"));	
				}
			break;
			case 'button_hide';	
				$sql="UPDATE sp_memberproduct SET is_open=0 WHERE id IN(".$id.")";
				if(spClass("member_product")->query($sql)){
					$this->error("批量取消推荐成功!",spUrl("member","memberproduct"));	
				}
			break;
			case 'button_show';	
				$sql="UPDATE sp_memberproduct SET is_open=1 WHERE id IN(".$id.")";
				if(spClass("member_product")->query($sql)){
					$this->error("批量推荐成功!",spUrl("member","memberproduct"));	
				}
			break;
		}
	}

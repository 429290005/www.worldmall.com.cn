...
function AddGbook($add){
	...
	$company_name = RepPostStr($add[company_name]);
	if(empty($bid)||empty($name)||empty($email)||empty($company_name)||!trim($lytext))
	{
	...
	$sql=$empire->query("insert into {$dbtbpre}enewsgbook(name,email,`call`,lytime,lytext,retext,bid,ip,checked,userid,username,company_name) values('$name','$email','$call','$lytime','$lytext','','$bid','$ip','$br[checked]','$userid','$username','$company_name');");
